...
class BlogCategoryService {
    ...
    async create(categoryData) {
        try {
            ...
            await this.invalidateAllCategoryCaches(category._id, category.slug);
    ...
    async delete(id) {
        ...
        // PERFORMANCE: Invalidate all category-related caches
        await this.invalidateAllCategoryCaches(id, slug);

        return true;
    }

    // PERFORMANCE: Comprehensive cache invalidation for categories
    async invalidateAllCategoryCaches(id, slug) {
        try {
            await Promise.all([
                cacheService.delPattern('blog_categories:*'),
                cacheService.delPattern('blog_category:*')
            ]);
            console.log('✅ All blog category caches invalidated');

            // 🔄 Frontend Revalidation
            if (slug) {
                await callFrontendRevalidation('blog_category', slug);
            }
        } catch (error) {
